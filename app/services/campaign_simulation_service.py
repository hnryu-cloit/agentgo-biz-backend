...
class CampaignSimulationService:
    ...
    async def simulate_bep(
        ...
        *,
        ...
        return await self._post_to_ai("/api/v1/campaigns/simulate-bep", payload)
    ...
    async def _load_metrics(self, store_key: str) -> tuple[float, int, float, float, datetime]:
        ...
        if latest_sales_date:
            start_date = latest_sales_date - timedelta(days=13)
            pos_rows = list(
                (
                    await self.db.execute(
                        select(PosDailySalesSnapshot).where(
                            PosDailySalesSnapshot.store_key == store_key,
                            PosDailySalesSnapshot.sales_date >= start_date,
                        )
                    )
                ).scalars().all()
            )
